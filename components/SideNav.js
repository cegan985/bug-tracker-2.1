...
function SideNav() {
  ...
  return (
        <div>
            <div onClick={() => setOpen(!open)} className={`${open ? 'left-64' : 'left-12'} hidden md:inline-flex duration-300 cursor-pointer hover:bg-gray-100 hover:dark:bg-slate-900 bg-white shadow-md dark:bg-slate-800 fixed z-50 border-b border-l border-r rounded-full justify-end border-gray-100 dark:border-slate-700 p-3 hover:shadow-lg`}>
                <ChevronDoubleLeftIcon className={`hidden text-gray-900 h-6 md:inline-flex cursor-pointer dark:text-white ${!open && 'rotate-180'}`}/>
            </div>
            <div className={`grid grid-cols-1 shadow-2xl justify-start bg-white dark:bg-slate-800  dark:border-slate-700 p-6 h-[calc(100vh-71px)] z-40 fixed -left-96 ${open ? 'w-72' : 'w-20'} lg:left-0 peer-focus:left-0 peer:transition ease-out duration-300`}>
                <div className='flex flex-col justify-start items-center'>
                    <div className='my-4 border-b border-gray-100 dark:border-slate-700 pb-4'>
                        <div onClick={() => router.push('/dashboard')} className={`${!open && 'w-12'} flex hover:scale-105 transition-all duration-150 ease-out active:scale-100 mb-2 justify-start items-center gap-4 hover:bg-gray-100 hover:dark:bg-slate-900 p-3 rounded-md group cursor-pointer hover:shadow-lg m-auto`}>
                        ...
                        <div onClick={() => router.push('/')} className={`${!open && 'w-12'} flex hover:scale-105 transition-all duration-150 ease-out active:scale-100 mb-2 justify-start items-center gap-4 hover:bg-gray-100 hover:dark:bg-slate-900 p-3 rounded-md group cursor-pointer hover:shadow-lg m-auto`}>
